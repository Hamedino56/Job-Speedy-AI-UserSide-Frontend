...
const AIToolsPage = () => {
  ...
  const handleGenerateJobAd = async () => {
    ...
    try {
      ...
      showSuccess(
        language === "de"
          ? "Stellenanzeige erfolgreich generiert"
      ...
      showError(
        err.message ||
          (language === "de"
            ? "Stellenanzeige konnte nicht generiert werden. Bitte versuchen Sie es erneut."
            : "Failed to generate job ad. Please try again.")
      );
    } finally {
  ...
  const handlePostJob = async () => {
    ...
    try {
      const requirementsList = (() => {
        const raw = generatedJob.requirements ?? generatedJob.required_skills ?? [];
      ...
      const data = await apiFetch("/api/jobs", {
        ...
        body: JSON.stringify({
          ...
          department: generatedJob.department || generatedJob.company || "General",
          description: generatedJob.description || "",
          requirements: requirementsList,
          status: generatedJob.status || generatedJob.job_type || "Open",
          created_by: "Admin",
        }),
      });
      ...
      if (data && data.job) {
        ...
        showError(
          language === "de"
            ? "Veröffentlichen der Stelle fehlgeschlagen"
            : "Failed to post job"
        );
      }
    } catch (err) {
  ...
  const handleMatchCandidates = async () => {
    if (!selectedJob) {
      showInfo(
        language === "de"
          ? "Bitte wählen Sie eine Stelle aus"
          : "Please select a job"
      );
      return;
    }
    ...
    try {
      const data = await apiFetch(`/api/jobs/${selectedJob}/applications`);
      const list = data.applications || [];
      setMatchedCandidates(list);
      if (!list.length) {
        showInfo(
          language === "de"
      ...
      showError(
        language === "de"
          ? "Kandidatenabgleich fehlgeschlagen"
  ...
  const handleExtractSkills = async () => {
    if (!uploadedFile) {
      showInfo(
        language === "de"
          ? "Bitte laden Sie zuerst einen PDF-Lebenslauf hoch."
    ...
    if (uploadedFile.type !== "application/pdf") {
      showError(
        language === "de"
          ? "Bitte laden Sie eine PDF-Datei hoch."
    ...
    try {
      ...
      const res = await apiFetchRaw('/api/tools/extract-skills', {
        method: 'POST',
        body: form,
      });
      ...
      if (!res.ok)
        throw new Error(
          payload.error ||
            (language === "de"
              ? "Extraktion der Fähigkeiten fehlgeschlagen"
  ...
  const handleDownloadSkills = () => {
    ...
    showSuccess(
      language === "de"
        ? "Skills-Datei heruntergeladen"
        : "Skills file downloaded"
    );
  };
